...
<body>
    <div class="page-wrapper">
        ...
        <header class="main-header header-one">
            ...
            <div class="header-upper pt-0 py-10 rpy-0">
                <!-- Header Ribbon  -->
                <div class="header-ribbon align-items-center">
                    <div class="inner-container">
                        <div class="d-flex justify-content-end  align-items-center">
                            <!-- Phone Number -->
                            <div class="d-flex float-right  align-items-center">
                                <a href="tel:+919480022108" class="text-decoration-none text-dark me-1">
                                    <i class="fal fa-phone-volume  "></i>
                                    <span class="d-inline d-sm-inline">94800-22108</span>
                                </a>
                            </div>
                        </div>
                    </div>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($recaptcha_response)) {
            // No reCAPTCHA token - block submission
        ...
        if ($verify_response === false) {
            // Could not connect to Google - block submission
            header("Location: index.html");
            exit();
        }
        ...
        if (!$response_data || !isset($response_data->success) || !$response_data->success) {
            // reCAPTCHA verification failed - block submission
            header("Location: index.html");
            exit();
        }

        // Check reCAPTCHA score (0.0 = bot, 1.0 = human)
        $recaptcha_score = isset($response_data->score) ? $response_data->score : 0;
        if ($recaptcha_score < 0.5) {
            // Bot detected (low score) - block submission
            header("Location: index.html");
            exit();
        }
        ...
        // Handle both phone_number and phone field names
        $number = '';
        if (isset($_POST["phone_number"]) && !empty($_POST["phone_number"])) {
            $number = htmlspecialchars(trim($_POST["phone_number"]));
        } elseif (isset($_POST["phone"]) && !empty($_POST["phone"])) {
            $number = htmlspecialchars(trim($_POST["phone"]));
        }

        $arrival = isset($_POST["arrival"]) ? htmlspecialchars(trim($_POST["arrival"])) : '';
        $deperture = isset($_POST["departure"]) ? htmlspecialchars(trim($_POST["departure"])) : '';
        $adult = isset($_POST["adults"]) ? intval($_POST["adults"]) : 0;
        ...
        $messageAdmin .= "Departure Date: $deperture\n";
        ...
        $headersUser = "From: Kabini River Lodge <book@kabiniriverresort.com>\r\n";
        $headersUser .= "Reply-To: book@kabiniriverresort.com\r\n";
